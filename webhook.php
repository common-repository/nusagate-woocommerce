...
class Nusagate_Webhook {
    ...
    */
    public function complete_payment($request)
    {
        ...
        if ($json_payload->status == 'FAILED') 
        {
            $order->update_status( 'failed', 'Nusagate Webhook (Charged)' );
        }

        if ($json_payload->status == 'VOID') 
        {
            $order->update_status( 'canceled', 'Nusagate Webhook (Charged)' );
        }

        if ($json_payload->status == 'EXPIRED') 
        {
            $order->update_status( 'canceled', 'Nusagate Webhook (Charged)' );
        }
